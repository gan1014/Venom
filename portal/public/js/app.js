(() => {
  ...
  function trackMax() {
    ...
    if (!track) return Math.max(1, document.documentElement.scrollHeight - window.innerHeight);
    return Math.max(1, track.offsetHeight - window.innerHeight);
  }

  function readScroll() {
    target = clamp(window.scrollY / trackMax(), 0, 1);
  }
  ...
  preload().catch(() => {
    bootStatus.textContent = "Some plates failed — continuing";
    enterBtn.hidden = false;
  });
})();
